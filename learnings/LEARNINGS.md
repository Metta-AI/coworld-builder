...
- **`upload-coworld --wait-hosted-smoke` returns before the canonical flag flips.** The hosted *smoke* passing is what releases the CLI; the hosted *certification* that follows is what sets `canonical: true`, minutes later. Three consecutive uploads printed `Canonical: no / Hosted certification: certifying` and all became canonical shortly after — a version bump can never fix this, so the triage row "canonical false → bump" loops forever on it. Fix (now in `templates/coworld-release.yml`): a "Wait for the uploaded version to become canonical" step between upload-coworld and secret-put that polls and rewrites `upload.json` before "Enforce canonical" reads it.
- **A raw HTTPS GET of `/v2/coworlds/<cow_id>` with the bearer token is 403 from a GitHub runner** (every poll for 20 min), while the identical request succeeds from the sandbox and the same job's CLI calls authenticate fine. Poll through `uvx --from "$COWORLD_PKG" coworld status <cow_id> --json` (skip the docker-context banner before the JSON; needs no docker).
- **A forked mummy `websocketHandler` must keep coworld-ctf's `Ping → Pong` branch.** Without it, local certify fails `smoke-episode` with `game_contract_violation: "Game websocket did not answer a WebSocket Ping with Pong"`. New triage row material for prompts/40 — the failure names the fix.
- **Replay playback must re-apply seat joins at their recorded ticks.** `simFromReplay` seating both players at construction auto-starts playback at `startWaitTicks` while the live server started when the seats actually connected: any join later than the grace period hash-mismatches from the next tick and re-derives a different game. coworld-ctf's `replays.nim` does this correctly; the fork dropped it. Test it with a *late* join recorded through the server's own loop at the shipped `startWaitTicks` — helpers that set `startWaitTicks=0` mask the defect completely.
...
## 2026-08-28 hide-and-seek

- `git push` over HTTPS lost auth mid-session while `gh api` kept working (coordinator + three sub-agents all hit it): replay commits through the Git Data API — blobs → tree (with `base_tree`) → commit → `PATCH refs/heads/main` — then verify `origin/main^{tree} == main^{tree}` and reset local onto it. The PATCH can 422 `Update is not a fast forward` when another cron pushes in the window: refetch, rebase, rebuild the tree, PATCH again. Base64 a big binary (a png) into a `--input` payload file, never into argv (`Argument list too long`).
- Hosted pod startup is slow: a cert fixture with `lobbyJoinTimeoutTicks: 600` (25 s) fails hosted smoke with `player slot N never joined the lobby` while local certify passes (only 1–2 of 6 seats had joined at 25 s). Ship a 200 s join cap (4800 ticks) in the cert fixture AND the league variants from the first release, and re-check the design's worst-case cadence still clears the engine stop.
- Policy uploads never dedupe across releases: the image digest is part of policy content, so every `coworld-release.yml` dispatch mints a new `vN` for every policy even when no prompt changed. Read the champion/filler labels from the LATEST `release-result.json` and resolve UUIDs for that vN.
- The atlas merge queue is backing up: 12+ atlas PRs open (one per run since 08-24), so every new run's dispatch 1 fails on the whole unplaced backlog and re-places it — PRs 20690 and 20691 both place ~43 slugs, at different coordinates, and both independently chose 202,270 for their own dot. Cleanest landing for the human: merge the NEWEST atlas PR (it is a superset placed self-consistently) and close the older ones. Until the queue drains, expect dispatch 1 to fail and budget the `extra_cities` dispatch.
- A dead coordinator's builder thread can keep working: on a resume into phase 20, read the coworld repo's `pushedAt` and recent CI runs before dispatching a second builder — this run's original builder drove CI to green three hours after its coordinator died, and a duplicate would have raced it.
- `IDEA_PLACES` in `places.mjs` carries human-picked coordinates for shipped ideas (13 of this run's 43 backlog placements) — reuse them for `extra_cities`; run `atlas_spot.py` against a working copy you append each accepted pick into, so clearances account for earlier picks.

## 2026-08-28 snake-royale

- **The Ping→Pong fork loss is now a pattern, not an incident** (lux-ai 0.1.0, snake-royale 0.1.0): forks of coworld-ctf's `server.nim` keep losing the `Ping → Pong` branch in `websocketHandler`, and certify fails `game_contract_violation` on `/global`. And the obvious fix has its own trap: adding the Ping branch with a `message.kind != TextMessage: return` guard drops the player's **binary** registration frames — ctf-lineage players register via `BinaryMessage` (docker-smoke red: "seat N never registered — playing coil"). Restore the branch, guard nothing else. Added to the make-coworld gotcha table.
- **Bedrock haiku's latency tail now brushes a 6 s first-attempt deadline** (round p50 2.1 s / p90 5.1 s / max 6.6 s vs `attempt1Ms=6000`): the check-5 log-cleanliness gate becomes a per-round coin flip (our round 2 had 3 fallbacks, round 3 was CLEAN), and the starter reports a transport timeout as `cause=parse_error` in the fallback record — misleading triage. Designs setting per-call deadlines should size attempt1 ≥ ~8 s if the turn budget allows, and a timeout should be labelled `timeout`, not `parse_error`.
...
## 2026-08-28 derks-gym

- **Player-side LLM needs `USE_BEDROCK: "true"` regardless of lineage.** The cogolf gotcha (2026-08-24) is filed under "factorio lineage" but applies to ANY policy whose `PLAYER_PROMPT` player makes its own LLM calls — this moba-lineage run hit it identically. `ANTHROPIC_API_KEY_URI` is not materialised in player pods; only the sidecar (gated on `USE_BEDROCK` in the policy env) provides a transport (`AWS_ENDPOINT_URL_BEDROCK_RUNTIME` + `AWS_BEARER_TOKEN_BEDROCK`). The tell in evidence: draft/decision records with `decision_ms` ≈ 1 and outputs byte-identical to the scripted fallback's table while the episode otherwise plays fine. Copy `cogame-factorio/players/llm_player.py`'s `provider_from_env()` + `_BedrockHttpClient` shape from day one, and put `USE_BEDROCK` on the manifest's LLM runnable too.
- **Once the registry has rows, a starter-inherited push-triggered `upload-coworld` job races `coworld-release.yml`.** derks-gym 0.1.1 was push-published mid-release (409ed the release's own upload), became CANONICAL, and was never locally certified, had no policy versions and no secret. Gate the job on the `UPLOAD_REQUIRED` repo variable in phase 20 (this run shipped the gate as commit cad9bd49); publishing belongs to the release workflow alone.
- **`viewer_smoke.mjs` now probes both chrome lineages.** The moba/emscripten lineage names its scrubber `#seek` and prefixes readouts (`#derk-clock`); the instrument previously probed only `#scrub`/`#clock`, so check 8 returned loaded:true with every readout null. Fixed in `templates/tools/ci/viewer_smoke.mjs` (selector fallbacks `#seek`, `[id$="-clock"]`, `[id$="-scorebug"]`, `[id$="-feed"]`, `input[type=range]`).
- **A player that awaits a slow LLM call inline in its websocket read loop gets its socket closed by the server's heartbeat** (aiohttp cannot answer PINGs while the coroutine is blocked; close at ~1.5× heartbeat). Run the decision as its own task and keep the read loop servicing the connection. Found empirically by the r2 reviewer at scaled timings.
- **Observatory API list endpoints flip between a bare array and an `{entries:…}` envelope** — same endpoint, different minutes (`/rounds` did both within 20 min). Wrap every list read in `jq 'if type=="array" then . else .entries end'`.
- **Hosted `episode-requests/<id>/artifacts/logs` carries game-pod containers only** (coworld-init-config, bedrock-sidecar, game, worker). Player stderr is unobservable, so any player-side contract ("the fallback is recorded") must ALSO surface in game-side records — here the server-measured `decision_ms` + draft records were the usable evidence.
...
## 2026-08-28 atari-57
- **git-over-HTTPS pushes to github.com started returning 401 mid-run** (both `gh auth git-credential` and explicit `x-access-token:$GH_TOKEN` URLs) while `gh api` with the same token worked and showed `push:true`. Fix that worked for coordinator and fixer alike: replay local commits via the Git Data API — blob → tree (`base_tree` = parent's) → commit → non-forced `PATCH /git/refs/heads/main`. Keep messages and order; never squash. A reusable helper lived at `/tmp/gitdata_push.py` this run; recreate it from the log if needed.
- **Atlas unplaced-leagues gate, at fleet scale**: with dozens of shipped-but-unmerged atlas PRs queued, every new dispatch fails `step=build` until it carries *all* unplaced slugs. Don't re-derive placements — mirror the **newest queued atlas PR's** added CITIES lines verbatim as `extra_cities`, add spots only for slugs it misses, and **re-spot your own dot against the augmented places.mjs**: my first spot (202,270 paintlands) was byte-identical to derks-gym's queued dot because atlas_spot only sees main. Also check the newest PR for continent re-calls: it moved atari-cabinet from paintlands (its own STATE) to simulations, so benchmark-suite games (atari-*, minigrid, procgen, flatland, rware…) now live in The Great Simulations.
- **Round 1 can fail on a trigger/unpause race** even with fillers registered first: `Temporal RoundWorkflow failed before settling the round.` on round 1, while round 2 (the scheduler's own) completed cleanly with both champions seated. Don't re-trigger in a panic; wait one interval and count from the next round.
- **A builder sub-agent outlives a dead coordinator session.** Phase 20's builder finished both CI runs green ~2.5 h after the coordinator died. On resume, verify the repo's exit criteria before re-dispatching a builder round — this run's phase 20 completed itself.
- Observatory v2 `/leagues`, `/rounds`, `/policy-versions` return **bare arrays** (not `{entries:[…]}`); flat `/episode-requests?round_id=` now **405s** — use nested `/rounds/<id>/episode-requests`.
- `viewer-check.yml`'s `feed_lines` probe can read 0 while the feed visibly renders (selector mismatch on this coworld's feed element). Judge the feed from viewer-smoke.png; consider fixing the probe's selector fleet-side.

## 2026-08-28 procgen

- **Ship hosted-scale LLM deadlines from day one, even single-seat.** Design said `attempt1Ms 5000 / retryMs 2000`; hosted Bedrock haiku ran p50 ~2 s, p90 5.6–7.5 s, max 9.2 s on ~1.7k-token prompts, so every round tripped the check-5 `falling back` grep while every call was 200-OK. 10000/5000/16000 cleared it (0.1.1). The ≤720 s bound must come from the wall-clock budget guard, not `turns × turnBudgetMs` — with wide deadlines the product is meaningless.
- **Assistant prefill `{` ends the prose-before-JSON `cut off at max_tokens` failure by construction** (0.1.2): send `[{user},{assistant:"{"}]`, re-prefix in `textOf`, and guard against a provider echoing the prefill (`{{`). Keep `maxOutputTokens` 900 — raising the cap just trades max_tokens fallbacks for timeout fallbacks. A truncated-but-started reply lands on the repair path, which is not a gated string.
- **Label a transport timeout `timeout`, not `parse_error`** — the wrong cause label sent triage toward the parser when the fix was deadlines.
- **After a mid-run re-release, rounds are not on the new image until scheduled after canonical**: rounds 7–9 mixed 0.1.1/0.1.2 episodes. Read each episode's `coworld_id` from `GET /episode-requests/<id>` before judging check 5 on it; a dirty log from the old cow_id is not evidence against the fix.
- **Re-released policies do not dedupe when the image changed** — every re-release minted v2/v3 for byte-identical prompts. The league keeps the seated v1 UUIDs; reseating is only needed if the fix lives in the *player* container (here it was game-container-side: decide/llm compile into the game server only).
...
## 2026-09-03 battlecode

- **git-over-HTTPS push died mid-session, sandbox-wide** ("Invalid username or token"), after working for hours. Pushes to coworld-builder AND coworld repos now go through the Git Data API (`gh api`: blobs → tree → commit → `PATCH refs/heads/main`, `force=false`). Two gotchas: an empty repo 409s (seed one commit via the contents API first), and big binary blobs overflow ARG_MAX — pass blob JSON via `--input` file, never `-f content=`.
- **Certifier contract probes belong in CI from day one.** Three release dispatches (0.1.1–0.1.3) were spent discovering, one 10-minute round-trip at a time, what `coworld certify`'s smoke-episode checks: a bad seat token must be REFUSED pre-upgrade; `/global` must answer a WebSocket upgrade and send its first frame immediately on connect; a Ping's Pong must ECHO the ping payload (`websocket.send(message.data, Pong)` — ctf does this; an empty Pong passes a structural review and fails the certifier, RFC 6455 §5.5.3). Fix shipped: `tools/ci/cert_probe.py` lifts the probes verbatim from the pinned coworld wheel and docker_smoke runs them against the real image. Lift it into `templates/`.
- **`config_schema` must declare AND require `tokens`** — the runner injects per-seat connection tokens into every episode's `game_config`, so `additionalProperties:false` without a `tokens` property fails `matriculate`. The playbook's "no tokens in `game_config`" means shipped *values*, not the schema.
- **`<IMAGE>` for policies = the *player* service's image** in a two-service compose (`upload-policy` cuts from it); `cogame-<slug>` alone matches nothing `compose build` produces and fails at "Docker image is not available locally".
- **Re-releasing without minting policy versions:** `-f policies='[]'` (the literal two characters) cleanly skips upload-policies; `-f policies=''` is the default and mints new versions of everything. League seeds resolve the canonical coworld **by name**, so a version bump needs no league rebind, and existing policy versions keep working when the player binary is unchanged (`cow_id` is per-version, not per-coworld).
- **Mechanical episode checks pass degenerate matches.** Round 1 had fallbacks [0,0], zero defaults, clean logs — and one champion had sheet-picked a do-nothing chassis and won because the opponent's kings starved. Two rules for future designs: never expose a knob that lets an LLM select an inert/weak chassis (the strategy surface must be inside one competent chassis), and give check 4 a substance assertion (both seats' summary stats non-trivial: units built, resources moved, damage dealt) so an idle-win is machine-visible.
- **A distilled-bot port needs an economic-survival gate**, not just knob-sensitivity: awu's kings died of *starvation* (partial dirt shell at ruinous dig cost, squeak beacon pulling cats, crowning beyond cheese income, buried-at-spawn on one map). Gate shape that worked: self-play on N maps must reach the round limit or end on points in ≥4/5, none losing all kings before round 1500 — and it must FAIL on the pre-fix chassis.
- **Atlas backlog:** 62 shipped leagues had no dot (build refuses until every league is placed). One dispatch fixed all of them via `extra_cities` (spots from `atlas_spot.py` run iteratively against a locally-growing places.mjs so batch placements don't collide) plus `drop_slugs` for stale lines — pre-check stale slugs by diffing CITIES against `/api/coworlds` before spending a dispatch.
- **Multi-year league shape (operator pin, worked):** seed with `league_key`/`league_name`/`default_variant_id` = the year key, then `POST /leagues/$L/short-name` and `POST /games/$GAME/default-league`. `/leagues` returns a bare array (no `.entries`), as does `/policy-versions`.
- **viewer_smoke.mjs SCRUB_SELECTOR** (`'#scrub, #seek, input[type="range"]'` + `.first()`) resolves in DOM order and hits a viewpanel zoom slider before `#scrub` — the smoke then measures zoom clicks, not seeks. Fix the template selector to prefer `#scrub`/`#seek` and exclude `#zoom-slider`.
- Round rows can be created seconds BEFORE the filler registration lands (round 1 at :02, fillers at :30) — count "rounds after fillers" strictly, and expect the phase-60 replay schema to differ per game (here: `result` singular, `events[].kind`).

### 2026-09-03 battlecode — addendum (operator correction, 2026-09-04)
- **OPEN ITEM, not complete:** the parity oracle is bit-exact only on Tiers A/B and on 4 of 6 full-game traces. The Tier C first-divergences — `arrows` round 915, `dirtfulcat` round 453 — shipped in docs/PARITY.md as a baseline **without a root cause**, which the operator ruled an unacceptable close state for a full behaviour port. Root-causing is tracked as Fleet card 1218171523823317; until it lands, do not cite this run's parity as "complete", and a future Battlecode-year run must budget for root-causing long-horizon divergences (the mechanism is likely a rare code path — trap/carry/tiebreak — that only fires mid-game).
- **Process defect that caused the miss:** an operator comment posted mid-run (01:25Z) was only discovered after close, because run-task comments were read at heartbeat start only. Re-read the run task's comments at EVERY phase transition, not just at session start — a phase transition is a natural checkpoint and costs one GET.

## 2026-09-04 battlecode-2020-soup
- **Two runs, one repo, works — with three disciplines.** A year-module MOD run on a repo whose sibling run is still live: (1) branch-only until the sibling's STATE says phase 80/RUN COMPLETE, then merge (checked its STATE/log for freshness only, never its task); (2) expect the sibling to spend shared resources — it took GameVersion GV04 mid-build, so the branch rebased and shipped GV05 with ReplayCompatibleGameVersions ["GV04","GV05"]; (3) release with a `policies` dispatch override limited to the new year's entries, or the release mints unused vN+1s of the sibling's league-seated policies.
- **Second league on one game (one league per year):** seed with `league_key:"<year>"` + `default_variant_id:"<year>"`, POST `short-name` so `softmax.com/<coworld>/<year>` addresses it, and do NOT touch `default-league` (that is the first league's). `GET /leagues` filtered on `game.coworld_name` now returns multiple rows — select by league name/key, or you will configure the sibling's league. `/api/coworlds` lists the sub-league as `<coworld>/<year>` — that is the atlas slug.
- **Design-note contradiction the checklist should catch:** "manifest `player[]` gains entries" + "certification fixture unchanged" is impossible when `len(certification.players) == num_agents` — the certifier requires EVERY declared player[] entry to occupy a cert slot (gotcha table line 396 already said so; the designer and the coordinator both missed the cross-check). Cost one release dispatch (`players_missing`). Fix was to drop the redundant entries: per-year baseline resolution (`awu`→year's strong baseline) made them unnecessary. Also fixed: `docker_smoke.sh` silently seated a player with NO env when `SMOKE_PLAYER_IDS` named an undeclared id — now a SEAT-COUNT FAIL in cogame-battlecode; candidate for `templates/tools/ci/docker_smoke.sh`.
- **viewer_smoke.mjs scrub clicks resolved by DOM order:** `'#scrub, #seek, input[type="range"]'` as one comma list clicked the shell's `#zoom-slider` (an input[type=range] earlier in the DOM), freezing the clock readouts and failing check 8 on a healthy viewer. Fixed in `templates/tools/ci/viewer_smoke.mjs` (preference-order resolution, first present-and-visible). Coworld repos scaffolded before 2026-09-04 carry the old copy in their own CI — harmless unless their zoom slider precedes #scrub. The verifier's controlled test (re-run with `?viewpanel=0`, scrub array collapsed to one entry) is the model for proving an instrument defect vs a product defect.
- **Atlas while a sibling atlas PR sits in the merge queue:** main's places.mjs lags every queued PR, so the build fails "unplaced leagues" for 60+ shipped coworlds. Mirror the queued PR's added CITIES entries VERBATIM into `extra_cities` (and its drop_slugs), and run `atlas_spot.py` against the PR's places.mjs, not stale main — the first spot main suggested was exactly where the queued PR had already put paintbot/campaign. Identical hunks rebase cleanly whichever PR lands first.
- **git push over HTTPS refused sandbox-wide** ("Invalid username or token") while the same token's API works: replay commits via the git-data REST API (blobs → tree with base_tree → commit → non-force ref PATCH; a rejected PATCH = lost race, same semantics as a rejected push). Script pattern in this run's log; the builder independently hit and solved the same thing.
- **Open items handed forward:** (1) killfeed panel overdraws the bottom-right stat boxes at FIT zoom (judge: advisory) — fix in the next cogame-battlecode version bump; (2) the 2020 full-engine oracle stays narrowed (upstream `net.sf.jsi` artifact is dead) — dependency-free vector parity (67,559 lines) is the shipped gate, documented in docs/PARITY.md §bc20; (3) eight more battlecode-year ideas are queued and were deferred by other heartbeats while this run held the repo — they inherit all of the above.

## 2026-09-04 battlecode-2021
- **Resuming at the 180-min staleness floor while the dead session's builder thread is still alive:** sub-agent threads survive their coordinator, and this one pushed the whole bc21 module 3 h after dispatch, seconds before the resume. When the adopted run's phase has a live builder (fresh commits/CI on its branch), MONITOR — watch its CI, let it fix red runs and merge — instead of re-dispatching a second builder onto the same branch. It fixed both red-CI causes and merged PR #3 unaided; the coordinator's job reduced to exit-criteria verification.
- **JDK-8 `javac` has no `--release` flag** (JDK 9+). A parity-oracle job that pins Temurin 8 must compile with plain `javac` flags (or `-source/-target`); `--release 8` dies with "invalid flag" in seconds. Cost one CI round.
- **Playwright `page.evaluate(SCRIPT_STRING)` silently returns `undefined` unless the string is an IIFE** (`(()=>{…})()`): a `(()=>{…})` closure without the call serialises as an uncalled function and every downstream `.ok === false` filter no-ops — the gate looks armed, runs green, and tests nothing. Sibling script constants ended `})();` and the new one ended `})`. Reviewer proved it three ways (playwright-core source, serializeAsCallArgument on a function, end-to-end). When adding a viewer gate, add a NEGATIVE CONTROL run (break the CSS, assert the gate goes red) in the same commit — that is what the r1 fix shipped.
- **Compute-heavy replays defeat fixed-wait scrub probes.** bc21 costs 3.12 ms/round native (18× bc20); check 8's three clock readouts were identical because the Worker's seek re-simulation outlives a 700 ms settle — loaded:true, viewer healthy, instrument too impatient. `viewer-check.yml` now takes `settle` (adaptive poll, records real `settle_ms` per seek) and `soak` (unattended-playback proof) inputs; on any sim whose docker-smoke `sim_seconds/rounds` exceeds ~1 ms/round, dispatch check 8 with `settle=20000 soak=15` from the start. bc21 seeks land in 3.5–4 s — spectator-visible; future heavy year modules should consider Worker-side keyframe checkpoints.
- **Re-releasing with the repo's full policies.json recuts EVERY policy as vN+1** (the content hash includes the rebuilt image digest): bc26/bc20 entries became v2 alongside the new bc21 v1s. Harmless — leagues hold their seated version UUIDs — but phase 50 must use the labels from THIS release's release-result.json, never remembered ones. (The bc20 run avoided this with a `policies` dispatch override; both shapes work, pick one deliberately.)
- **bc21 parity closed honestly but narrower than designed:** Tier A bit-exact windows are sized by the JVM's own bytecode cut-off (22–245 rounds), the design's Tier B trace never existed, and the accepted-divergence ledger has 5 root-caused entries. All disclosed in docs/PARITY.md + RULES-BC21.md (r1 F2 made the docs truthful — the job MEASURES the bytecode boundary, it does not assert 80%). A future year module should pin parity tiers to what the JVM harness can actually deliver before the design note promises an empty ledger.

## 2026-09-07 battlecode-2025
- **`tools/atlas_spot.py` misparses CITIES labels containing bare digits** (its number regex takes the first integer in the entry, so `["battlecode","Battlecode 2026",331,246,…]` is read as a dot at (2026,331)). When batch-spotting with a locally-extended places.mjs (the extra_cities workflow), append placeholder entries with digit-free slug/label text; the real labels go only in the JSON. Symptom if you miss it: two leagues assigned the identical "roomiest" point.
- **Atlas phase now fails structurally, not incidentally:** main's places.mjs is missing 68 shipped coworlds (everything since ~08-24) and carries stale lines whose leagues fetch-stats drops — including `paintbot/ctf`, which is still in `/api/coworlds` (payload-shape regression in fetch-stats, needs a human per prompts/75-atlas.md step 8). Give-up path taken; the spaced, ready-to-use extra_cities JSON for all 68 is committed at `runs/2026-09-07-battlecode-2025/atlas-extra-cities.json` — the next run (or the human landing Fleet card 1218254195545568) should reuse it instead of re-spotting, adding `drop_slugs=paintbot/classic,paintbot/ctf` if ctf's page is ruled dead.
- **The bc24 reviewer-dies-silently failure (its standing Blocked cause) is mitigated by brief wording:** instruct the reviewer to CREATE the output file first with a header, append per-section, and bound its reading to the diff (open full files only for hunk context). The same 100+-file battlecode diff that killed three bc24 reviewer threads produced a 361-line, 43-finding review on the first attempt here. Apply the same instruction to every large-diff dispatch (judge and fixer included), and consider it for bc24's unblock.
- **Verify on a mod year whose match is best-of-N:** the episode legitimately plays fewer games than `gamesPerMatch` (clinch at ⌈N/2⌉, reason stays `complete`); also this coworld records `result` (singular) — `.results` is null. Read the year module's match loop before judging "too few games" a defect.
- **Beat emission ≠ beat CSS.** The bc25 diff shipped 11 beat-kind CSS rules and a CSS-presence test while `beatsFor` emitted 2 kinds (F26, the run's only blocking finding). The durable test shape: assert kinds are EMITTED from the committed fixture (≥N beats over ≥K kinds), labelled, and styled — emission, label, style, all three. bc24/bc21 still have the 2-beat gap; a future year run touching broadcast.nim should fix them in the same commit.
- **Landing a big file through the git Data API: never pass base64 as an argv flag.** `gh api -f content=<base64>` dies with "Argument list too long" above ~100 KB and, inside a `for` loop, silently drops that file from the tree while the ref PATCH still succeeds — this run landed its close commit without LEARNINGS.md and only a content re-read caught it. Always `--input` a JSON payload file for blobs, and diff the landed tree against the intended file list before resetting the local branch.

## 2026-09-08 battlecode-2023
- **A builder thread outlives a dead coordinator session — check before re-dispatching.** The phase-20 builder kept working 3+ h after the coordinator session died, pushed its second commit and opened the PR minutes after the next session resumed. The resuming coordinator monitored (branch commits + CI) instead of re-dispatching and avoided a duplicate; a parallel cron session (51417e08) that did re-dispatch a fixer noticed the original's return, stood its duplicate down, and yielded — the comment-first/yield discipline held under a three-way overlap. Takeover threshold that worked: CI complete-red with no new branch commit for 40 min.
- **Parity comparators must normalize BOTH traces identically.** parity_tiers_bc23.py stripped the `bc=` bytecode column from the Java trace only, while the Nim emitter writes a constant `bc=0` — every pair "diverged" at round 1 on otherwise-identical lines. Same class of latent bug found (not fixed, out of mod scope) in bc20/21/24/25 siblings: `zip` tail off-by-one (a one-line-longer Java trace reads bit-exact) and zero-padded `toHex` vs Java's `Long.toHexString` for shared-array folds. A future battlecode-year run should fix all siblings in one commit.
- **LLM champions sometimes wrap the doctrine sheet in a protocol envelope** (`{"protocol":…,"doctrine":{…}}`), sending all 12 knobs to `sheet_unknown_fields`: the seat silently plays the schema-default sheet, `defaults_applied` stays 0 (keys absent, not malformed), and the endcard renders the *applied* sheet as if the cog wrote it. Happened in 2 of the first 3 rounds (the winner won on defaults once). Future years: unwrap a single `doctrine`/`sheet` envelope key tolerantly before knob parsing, count absent-key defaulting in `defaults_applied`, and render submitted-vs-applied divergence on the doctrine card.
- **Atlas backlog is one dispatch now**: reuse runs/2026-09-07-battlecode-2025/atlas-extra-cities.json (68 leagues) + bc25@202,270 as extra_cities and drop_slugs=paintbot/classic,paintbot/ctf (both stale). PR 21888 places the whole backlog; injecting extra_cities into a local places.mjs copy before atlas_spot.py makes the tool avoid the incoming dots.
- **CI wall clock on cogame-battlecode is now ~65-75 min per round** (the test job alone 60 min with six year modules). Budget phase 20/30 rounds accordingly; the sibling bc24 run's phase-30 reviewer death was cushioned here by pinning incremental writes in every brief — all four sub-agent files (review/fixes/verdict/VERIFY) landed complete.
- **bc25's seven legibility findings partially recur on bc23** (bc26 nouns on the endcard, doctrine-card clipping) plus five new (raw float scores, endcard overflow at 1280x800 cutting the winner's stat block, HUD bleed-through, "a accelerating", empty mottos). None falsify the phase-60 gate; a future bc-year run should batch-fix the shared endcard template in replay_broadcast.html once for all years.

## 2026-09-08 battlecode-2022
- **`perGameBudgetSeconds = 0` does not mean "no budget" — it means ONE second.** `playMatch` computes `max(1, min(config.perGameBudgetSeconds, remaining))` (`src/battlecode/match.nim:480`), so a test helper that zeroes the field silently buys a 1-second per-game budget, while `years/bc2x/rules.nim` one level down *does* treat 0 as "unbounded" (`if budgetSeconds > 0`). The two layers disagree and the disagreement is year-neutral and pre-existing. Symptom is diagnostic: a 2000-round shard that **passes in `-d:release` and fails in debug** — release runs 2000 rounds in ~0 s and squeaks under the 1 s, debug takes ~4 s and aborts to `epDeadline`, then indexes an empty `games` seq and dies with `IndexDefect`. Copy `tests/test_bc23_replay.nim:69`'s `if perGame > 0:` convention rather than setting the field to 0, and guard every `games[0]` behind a non-empty check so a failed assertion prints a FAIL instead of a bounds crash (in release, bounds checks are off, so that read is unchecked OOB and the "pass" is not trustworthy either).
- **A debug-only test failure is a *behaviour* signal, not a flake.** Before reaching for `when defined(release)`, ask what the two builds do differently: here it was wall-clock, and gating the assertion would have hidden a real 1-second budget rather than fixing it. The fix that shipped touched one file, changed no engine code, added a guard, and left the shard with **more** checks than before (85 debug / 84 release vs 75).
- **`tools/atlas_spot.py`'s digit bug bit again, and the 2026-09-07 workaround was never applied.** The known misparse (its number regex takes the first two integers *anywhere* in the entry, so `"Battlecode 2020"` reads as x=2020) means every battlecode dot lands outside the region polygon and is dropped from the neighbour set entirely. Consequence this run: the tool's first answer for bc22 was `202,270 @ clearance 39.5` — bc25's exact reserved spot, true clearance **0**. It also explains bc23's and bc25's suspiciously roomy numbers. The durable fix is a **positional** parse of `[slug,label,x,y,region]`; until someone lands it, either use digit-free placeholder labels (the 2026-09-07 advice) **or** recompute clearance yourself over the unfiltered neighbour set — `atlas_spot`'s own `taken` list filters by `inside(poly)`, so a mis-parsed dot is invisible twice over.
- **Pre-empt the atlas build's failures in ONE dispatch instead of discovering them serially.** bc25 burned all three dispatches finding `unplaced-68` → `stale paintbot/classic` → `stale paintbot/ctf` one per run and ended `unplaced`. Passing the accumulated `extra_cities` + `drop_slugs` up front lands the PR first time. Two extra pins: **re-validate the inherited extra-cities file against `/api/coworlds` before reusing it** — `polyduel/v1` had left the directory since bc25 wrote the file and would have been a fresh stale-line failure — and remember that entries you add via `extra_cities` are neighbours too, so spot against places.mjs **plus** the extras, not places.mjs alone.
- **A mod run's announce/verify slug is not the run slug.** The coworld is `battlecode`; `softmax.com/battlecode-2022` does not exist. Phase 60 check 6 and the phase 70 play link both take `softmax.com/battlecode/bc22` (the league short-name route). Brief sub-agents with the API-visible slug explicitly or they will record a false negative on a page that was never going to exist.
- **The league count in a design note is a prediction, not a fact.** The note expected `GET /leagues` to return seven battlecode leagues; it returned five, because the bc24 run is Blocked and never created its own. Select the league by `league_key`/name client-side — a positional or count-based selection would have configured a sibling year's league.
- **`policies.json` on a multi-year repo is repo-wide.** It carries 4 entries per year (28 here). Dispatching the whole file recuts vN+1 of every sibling year's policies under six live leagues; filter to this year's four with a `policies` override and assert the filter returns exactly 4 before dispatching.
- **Still open, inherited forward:** `tests/test_bc23_replay.nim:132-144` asserts *strictly* `epDeadline` on a 2000-round game with a 1-second budget, sampled every 32 rounds — runner-speed dependent, and it failed on one push run this session while the identical tree passed on the PR run and on `main`. Give it bc22's tolerant `reason in [epDeadline, epComplete]` shape. Also unfixed: `match.nim:480`'s clamp itself, and `viewer_smoke.mjs` reporting `canvas_text.total: 0` on this renderer (the text-bounds check covers nothing here and must not be read as a pass).

## 2026-09-09 battlecode-2016
- **Killfeed / beat labels are rendered at VIEW time, not written into the replay.** `src/battlecode/broadcast.nim`'s `beatsFor*(doc: ReplayDoc, …)` takes a finished replay as *input* and is compiled into `bc_replay.wasm` (`replay-viewer/bc_replay.nim:93`, `Dockerfile.replay-viewer`), so no label string ever appears in `.replay` bytes — grepping a replay for `LAUNCHER`/`TRADE` returns zero in **every** version and can never demonstrate a label fix. Two consequences a future run must not get backwards: (1) the decisive evidence for a chrome fix is a *rendered* frame (viewer-check screenshot) reconciled against the raw event, never a byte grep; (2) the fix still needs a release, because the viewer bundle is keyed to `<cow_id>/<manifest_hash>` — and once released it applies **retroactively**, so older replays re-render with the corrected wording too. This coordinator briefed the opposite ("baked in at episode time"), the verifier disproved it and said so instead of manufacturing evidence, and the judge then re-proved it at source/bytes/render level: brief sub-agents so that contradicting the brief is the expected outcome when the brief is wrong.
- **A year module that *reuses* a sibling year's event kind inherits that sibling's vocabulary silently.** bc16 emits `duel` with bc22's exact `attackersLostThisRound` semantics, but `broadcast.nim`'s emitter had a comment declaring `duel` bc23-only and rendered bc23's `launchers lost` for every year — so a live bc16 killfeed described a mechanic bc16 does not have. Grep every beat kind your year emits against the *other* years' noun lists before phase 60, and pin it with a three-layer test: this year's label built from the event's own payload, the sibling years' renderings of the **same** event pinned byte-for-byte (so the fix cannot be reached by moving theirs), and a generalisation over every kind the year emits against a forbidden-vocabulary list with an anti-vacuity guard (`the same feed read as bc23 must trip the search`). 91 → 6045 checks, no assertion weakened.
- **`git push` over HTTPS breaks mid-session in this sandbox and it is not a token problem.** Symptom: pushes succeed for hours, then every push fails `remote: Invalid username or token. Password authentication is not supported for Git operations.` while `git fetch`, `git ls-remote` and `gh api` keep working. Cause is the credential helper pinned for github.com, `!gh auth git-credential`, which sends the token base64-encoded inside a Basic header so egress cannot swap it; `gh auth setup-git` re-adds the same broken helper. Fix, confirmed in three separate containers (coordinator + two sub-agents): `git -c credential.helper='' -c credential.helper=anthropic push` (the empty entry first clears the inherited list; `/usr/local/bin/git-credential-anthropic` ships in the image). Where even that is absent, land commits through the **Git Data API** — blob → tree → commit → ref, one commit at a time, parented on the real `main`, then fetch them back and diff the tree against what you tested locally. Never a force-push on either path. (The 01:25Z heartbeat that closed this run found *no* helper configured and pushed fine, so check before assuming either state.)
- **A round created seconds after the canonical flip still runs the OLD coworld.** Round 29 was created 37 s after 0.8.2 certified and its three episode requests all carried the 0.8.1 `coworld_id`. Never infer version from a timestamp: wait for the first round whose episode requests carry the new `coworld_id` (the payload also carries `coworld_version`), and give the verifier that wait as an explicit step 0 with the 75-minute bound as its stop. The nested route is the working one — `$BASE/rounds/<id>/episode-requests`; the flat `GET /episode-requests?round_id=` returns **405** (the stale line in `prompts/60-verify.md`, flagged by a judge and hit again here).
- **Commit every sub-agent artifact the moment it lands, before the next dispatch.** Two sessions of this run died holding uncommitted files and lost the r1 review (737 lines) and r1 fixes; both had to be reconstructed from log lines and the landed diff, and the authentic copies only survived because the session that *lost a resume race* pushed its untracked files afterwards. Writing "commit the write-up immediately, here is what it cost this run twice" into the r3 brief produced two commits (one at write time, one with the CI section filled in) and zero loss. Same instruction belongs in every reviewer/fixer/verifier brief.
