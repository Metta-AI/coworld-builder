# VERIFY — cogmud   (2026-08-24T05:49Z)

Verdict: **all-true** (8/8: checks 1, 2, 3, 4, 5, 6, 7, 8 TRUE)
...
**Pinning.** Checks **3, 4 and 5** are pinned to **round 4**
(`round_e3d754d0-d3ec-43ec-836f-6e54c9b44488`, episode request
`ereq_2fc0e53e-ddb5-4ec8-b047-f8ea01cb0d58`, replay
`f2ecf3e4-dc96-43b5-b3e2-1768038352c2.replay`) — the **latest completed round at the moment those
three checks were (re-)executed, 2026-08-24T05:47:53Z**. Round 5 existed at that moment but was
`pending`, so round 4 is the latest *completed* one; confirmation is pasted under check 3.

Checks **6 and 8** remain on the featured match the public page was serving when they ran
(`cogmud.r3.e1` → `ereq_1151194f-c9ea-4ce8-8d76-51f6b3af1d7b`, replay
`632e1ef6-7d66-4ad6-afd7-67aa2e772f97.replay`, fetched 05:27–05:29Z). That is correct and not a
mismatch: check 6 asks whether the page's featured match is served over the static route, and
check 8 asks whether *that* bundle actually renders — both are properties of the featured episode,
whichever round it comes from, and its replay parsed clean and drew with `loaded: true`. Check 5 is
a property of the verified round's hosted log, which is now round 4's.

**Retry note.** Checks 3, 4 and 5 were first run at 05:26–05:27Z against round 3, which was then the
latest completed round; check 5 came back FALSE there (one `falling back` line). Per
`prompts/60-verify.md` §Retry budget, "different round" is a documented retry approach, and the
ladder has since produced round 4. All three checks were re-fetched fresh against round 4 at
05:47–05:48Z and are recorded below with that evidence. **The superseded round-3 evidence is kept
verbatim in the Appendix** — its fallback line and parser diagnosis are LEARNINGS material, and its
check-4 counts remain valid history.
...
rounds were completed by 05:26:06Z — 21 minutes in, well inside the bound; a third (round 4) by
05:40:00Z.
...
and round **3**'s the same (pasted in Appendix A.2) — the "Baseline (N)" renaming only happens for a
...
*(Re-pinned to round 4. The superseded round-3 run of this check is kept verbatim in
Appendix A.1.)*

**Confirming round 4 is the latest completed round** (fetched 2026-08-24T05:47:46Z):
```bash
curl -sS "$BASE/rounds?league_id=$L&limit=20" "${AUTH[@]}" \
 | jq -c 'if type=="array" then . else .entries end | map({n:.round_number,s:.status,id:.id,created_at})'
```
```json
[{"n":5,"s":"pending","id":"round_9deb507d-d141-4b4e-ae7e-b9d10986ed0f","created_at":"2026-08-24T05:47:28.816849Z"},
 {"n":4,"s":"completed","id":"round_e3d754d0-d3ec-43ec-836f-6e54c9b44488","created_at":"2026-08-24T05:32:28.399374Z"},
 {"n":3,"s":"completed","id":"round_578a4ce5-cc9d-45a0-93b6-6b8cc8eef912","created_at":"2026-08-24T05:17:28.031799Z"},
 {"n":2,"s":"completed","id":"round_a8fbacd3-cc5b-4754-ac93-3a7647322d54","created_at":"2026-08-24T05:02:27.040927Z"},
 {"n":1,"s":"failed","id":"round_7281fe2e-3c0b-4c5a-affe-15a952c44dfa","created_at":"2026-08-24T05:02:00.730415Z"}]
```
Round 5 exists but is `pending`; the `max_by(.round_number)` over `status=="completed"` is
therefore **round 4**.

**Commands** (fetched 2026-08-24T05:47:53Z):
```bash
...
# R=round_e3d754d0-d3ec-43ec-836f-6e54c9b44488   (round_number 4)
curl -sS "$BASE/episode-requests?round_id=$R&limit=20" "${AUTH[@]}" \
 | jq -c '(if type=="array" then . else .entries end)|map({id,status,replay_url})'
```
```json
[{"id":"ereq_2fc0e53e-ddb5-4ec8-b047-f8ea01cb0d58","status":"completed","replay_url":"https://softmax-public.s3.amazonaws.com/replays/f2ecf3e4-dc96-43b5-b3e2-1768038352c2.replay"}]
```

```bash
curl -sS "$BASE/episode-requests/ereq_2fc0e53e-ddb5-4ec8-b047-f8ea01cb0d58" "${AUTH[@]}" \
 | jq '{status, replay_url, participants, participant_scores}'
```
```json
{
  "status": "completed",
  "replay_url": "https://softmax-public.s3.amazonaws.com/replays/f2ecf3e4-dc96-43b5-b3e2-1768038352c2.replay",
  "participants": [
    {
      "position": 0,
      "kind": "policy",
      "policy_version_id": "8b364f9b-56be-48ce-a3bd-dba2c068dac9",
      "policy_id": "ba4de339-92c7-45b7-ad18-8f69983dd41a",
      "policy_name": "co-gas-cogmud-commission-runner-relhalpha",
      "version": 1,
      "player_id": "ply_18302115-9fc9-482d-a2f3-f4c592bf9e57",
      "player_name": "relh",
      "is_filler": false
    },
    {
    ...
    },
    {
      "position": 3,
      "kind": "policy",
      "policy_version_id": "328aa67b-dd1d-49b2-b395-0c50b12fd267",
      "policy_id": "4708ddb7-aa14-4a88-be5c-5c08d2860fd8",
      "policy_name": "co-gas-cogmud-commission-runner-richard",
      "version": 1,
  ...
  "participant_scores": [
    { "position": 0, "score": 2.025 },
    { "position": 1, "score": 1.825 },
    { "position": 2, "score": 1.325 },
    { "position": 3, "score": 1.85 },
    { "position": 4, "score": 0.475 },
    { "position": 5, "score": 1.4 }
  ]
}
```
(Nothing elided — this is the whole body of the `jq` projection.)

**Note — outside players have joined.** Round 4 seated **four** non-filler policies: besides the two
champions, `relh` (`co-gas-cogmud-commission-runner-relhalpha:v1`) and `richard`
(`co-gas-cogmud-commission-runner-richard:v1`) entered the league of their own accord between
rounds 3 and 4. Only two filler seats were needed as a result. This does not affect the check —
the requirement is that the champions are named correctly and the fillers are fillers — and it is
independent corroboration that the coworld is live and playable by third parties.

**Status: TRUE** — `status == "completed"`; `replay_url` non-null
(`…/replays/f2ecf3e4-dc96-43b5-b3e2-1768038352c2.replay`); `participants` names **`daveey`**
(position 1, `cogmud-merchant:v1`, `is_filler:false`) and **`daveey-1`** (position 2,
`cogmud-broker:v1`, `is_filler:false`); the two filler seats (positions 4, 5) carry
`is_filler:true` and render as `Baseline` / `Baseline (2)` in the replay (check 4).

---

## 4. Replay bytes are valid and show the game — **TRUE**

*(Re-pinned to round 4's replay. The superseded round-3 run of this check — which also passed — is
kept verbatim in Appendix A.2.)*

**Commands** (fetched 2026-08-24T05:48:05Z):
```bash
curl -sSL "https://softmax-public.s3.amazonaws.com/replays/f2ecf3e4-dc96-43b5-b3e2-1768038352c2.replay" \
     -o /tmp/ep4.replay        # http=200 bytes=61086
jq -e . /tmp/ep4.replay >/dev/null && echo "strict UTF-8 JSON: ok"
python3 -c "d=open('/tmp/ep4.replay','rb').read(); d.decode('utf-8'); print('python strict utf-8 decode: ok, bytes=%d'%len(d))"
```
```
strict UTF-8 JSON: ok
python strict utf-8 decode: ok, bytes=61086
```
...
{"names":["relh","daveey","daveey-1","richard","Baseline","Baseline (2)"],
 "scores":[2.025,1.825,1.325,1.85,0.475,1.4],"coin":[25,10,29,10,11,0],
 "wealth":[25,17,45,18,59,0],"questPoints":[32,32,16,32,0,32],"delivered":[4,4,2,4,0,4],
 "robberies":[0,0,0,0,1,0],"robbed":[0,1,0,0,0,0],"turns":14,"maxTurns":14,"reason":"complete"}
```
```bash
jq -c '.names, .policyNames' /tmp/ep4.replay
```
```json
["Ratchet","Sprocket","Gasket","Widget","Rivet","Piston"]
["relh","daveey","daveey-1","richard","Baseline","Baseline (2)"]
```
Seat → player: 0 `relh`, **1 `daveey`**, **2 `daveey-1`**, 3 `richard`, 4 `Baseline`,
5 `Baseline (2)` — matching the `participants` positions in check 3. The two filler seats are the
only ones renamed `Baseline (N)`, confirming the filler list is still in force.

```bash
jq -c '[.events[]|.kind]|group_by(.)|map({(.[0]):length})|add' /tmp/ep4.replay
jq -c '[.events[]|select(.kind=="end")]' /tmp/ep4.replay
```
```json
{"act":84,"end":1,"start":1,"turn":15}
[{"kind":"end","turn":14,"text":"complete"}]
```
...
```bash
$ jq -r '[.events[]|select(.type=="decision")]|length' /tmp/ep4.replay
0
$ jq -r '[.events[]|select(.fallback==true)]|length' /tmp/ep4.replay
0
...
$ jq -c '[.events[]|select(.kind=="act")]|group_by(.seat)
         |map({seat:.[0].seat,acts:length,scripted:([.[]|select(.scripted==true)]|length)})' /tmp/ep4.replay
...
$ jq -r '[.events[]|select(.kind=="act" and (.seat==1 or .seat==2))] as $c
         | "champion_decisions=\($c|length) champion_fallbacks=\([$c[]|select(.scripted==true)]|length)"' /tmp/ep4.replay
champion_decisions=28 champion_fallbacks=0
```
Seats 1 (`daveey`) and 2 (`daveey-1`) are the champions: **0 fallbacks in 28 champion decisions
(0.0 %)** — every champion decision in this episode was LLM-authored. All four non-filler seats
(0–3, including the two outside players) are 0/14 scripted; seats 4–5 are the fillers and are
scripted 14/14 by construction, as fillers must be.
...
```bash
$ jq -r '[.events[]|select(.kind=="act" and (.seat==1 or .seat==2))|.intent]
         |group_by(.)|map({(.[0]):length})|add|tojson' /tmp/ep4.replay
{"buy":4,"give":6,"move":11,"none":1,"quest":1,"take":4,"wait":1}
```
Verbatim champion sentences, all `scripted=false`
(`turn seat intent reason scripted sentence [say]`):
```
0	1	take	ok	false	I pick up the rope.
0	2	take	ok	false	I pick up the rope here in The Chapel.   say="One rope secured. I need one more rope and two nails to complete my commissions."
1	1	move	ok	false	I walk to The Docks.
1	2	move	no_such_exit	false	I walk to The Smithy.
2	1	take	ok	false	I pick up the nails here.
2	2	move	ok	false	I walk to Market Square.
3	1	quest	ok	false	I ask Dockmaster Fen about salt.
3	2	take	no_such_item	false	I pick up the rope here in Market Square.   say="Widget, I'm buying nails and rope - paying fair coin. Smith Bram at The Smithy, what's your stock?"
4	1	buy	ok	false	I buy two nails from Dockmaster Fen.
4	2	move	ok	false	I walk to The Smithy.
5	2	buy	ok	false	I buy two nails from Smith Bram.   say="Two nails for six coin, that leaves me thirty-four coin and a rope in pack."
5	1	move	ok	false	I walk to Market Square.
…
11	1	give	ok	false	I hand Guildmaster Vell two salt for my commission.
11	2	move	ok	false	I walk to the Guildhall.
12	1	give	ok	false	I hand Guildmaster Vell two nails for my commission.
12	2	give	ok	false	I hand Guildmaster Vell two nails for my commission.   say="First commission complete, now seeking one rope for the second."
13	2	buy	ok	false	I buy one rope from Guildmaster Vell.   say="Completing my second commission now with this rope."
13	1	wait	waited	false	I wait here and watch the other cogs.
```
Free-form English sentences, public price-talk addressed to named cogs and keepers, commissions
actually delivered (`delivered = [4,4,2,4,0,4]`; `questPoints[1] = 32`, `questPoints[2] = 16`), and
the parser's `no_such_exit` / `no_such_item` outcomes fed back as legal no-ops exactly as
`design.md:249` describes. This is the game, played.

**Status: TRUE** — strict UTF-8 JSON under two parsers; `protocol == "cogmud.replay.v1"` matching
the game source and design note; `results.reason == "complete"`; champion fallbacks **0/28**, with
non-trivial non-scripted content.

---

## 5. Hosted game log is clean — **TRUE**

*(Re-pinned to round 4. The superseded round-3 run — the one FALSE result of this verification, with
its full diagnosis and fleet cross-check — is kept verbatim in Appendix A.3, because it is LEARNINGS
material.)*

**Command** (fetched 2026-08-24T05:48:18Z):
```bash
curl -sS "$BASE/episode-requests/ereq_2fc0e53e-ddb5-4ec8-b047-f8ea01cb0d58/artifacts/logs" \
     "${AUTH[@]}" "${ELEV[@]}" -o logs_r4b.raw          # http=200 bytes=122896
...
python3 declogs.py logs_r4b.raw logs_r4b.decoded
grep -nE 'falling back|LLM provider is unavailable|cut off at max_tokens|rejected' logs_r4b.decoded \
  || echo CLEAN
```
```
...
Containers present in the body (so it is the full log, not a truncated slice):
```
...
**Corroboration that the episode really ran on the LLM** (an all-scripted episode would also grep
clean, so this is checked rather than assumed):
```bash
$ grep -c 'bedrock_sidecar_complete' logs_r4b.decoded
56
$ python3 -c "…Counter of (ok,status_code) over bedrock_sidecar_complete records…"
bedrock_sidecar_complete ok/status: Counter({('true', '200'): 56})
rate_limited lines: 0
```
56 successful Bedrock invocations, every one `"ok":true,"status_code":200`, and **zero**
`bedrock_sidecar_rate_limited` warnings — no throttling, no provider degradation.

**Not the "scripted-fast episode" symptom.** The episode ran the full 14 turns over ~161 s
(`playbooks/make-coworld.md`: "An LLM game whose episodes finish in ~20 s is playing scripted"):
```
cogmud: turn 12 of 14 at 136s
cogmud: turn 13 of 14 at 149s
cogmud: turn 14 of 14 at 161s
…
cogmud: episode complete, shutting down
```

**Status: TRUE** — **zero** lines match
`falling back|LLM provider is unavailable|cut off at max_tokens|rejected` in the decoded hosted log
of the latest completed round's episode (`ereq_2fc0e53e-ddb5-4ec8-b047-f8ea01cb0d58`, round 4).
No exception was needed and none was claimed. Round 2's log (`ereq_c4466214-…`) is also CLEAN
(0 hits, Appendix A.3); round 3's had a single intermittent fallback line, recorded there.

---

## 6. The public page uses the static replay path — **TRUE**

*(Run at 05:27:48–05:27:57Z, when the page's featured match was `cogmud.r3.e1`. Not re-pinned: the
check is about the route the page serves its featured match over, which is a property of that
episode whatever round it comes from. Its replay is the one parsed in Appendix A.2 and rendered in
check 8.)*
...
replay verified in Appendix A.1/A.2 (`ereq_1151194f…`, `…/632e1ef6-….replay`), with both champions in
the matchup.
...
*(Dispatched 05:28:04Z against check 6's iframe `src`, i.e. the featured match `cogmud.r3.e1`. Not
re-pinned: this is the one check whose evidence is a CI artifact that cannot be re-derived, and it
answers "does the hosted bundle draw a frame" — a property of the bundle, which is the same
`sha256:83f70a…` manifest hash for every round. The replay it drew is the one parsed in
Appendix A.2: strict-JSON valid, `reason: "complete"`, 84 acts over 14 turns.)*
...
Ordered excerpts from `/tmp/ep.replay` — round 3's replay, the featured match this viewer drew;
the same bytes parsed in Appendix A.2:
...
7	1	buy	ok	true	I buy 1 rope from Smith Bram.        ← the one champion fallback (Appendix A.3)
...
| # | Check | Pinned to | Verdict | Key datum |
|---|---|---|---|---|
| 1 | ≥2 completed post-filler rounds | league | **TRUE** | rounds 2, 3 & 4 `completed`; round 1 `failed` pre-filler, excluded with its error |
| 2 | Both champions ranked | division | **TRUE** | daveey r1 / daveey-1 r2, `rounds_played: 2` each; fillers absent |
| 3 | Latest round's episode request | round 4 | **TRUE** | `ereq_2fc0e53e…` `completed`, replay_url present, pos 1/2 = daveey / daveey-1 |
| 4 | Replay bytes valid & show the game | round 4 | **TRUE** | strict JSON ×2 parsers, `cogmud.replay.v1`, `reason:"complete"`, champion fallbacks **0/28** |
| 5 | Hosted game log clean | round 4 | **TRUE** | `hits=0` → **CLEAN**; 56 Bedrock calls all `ok:true/200`, 0 rate-limited, 14 turns in 161 s |
| 6 | Public page uses static replay path | featured match `cogmud.r3.e1` | **TRUE** | `…/replays/static/cow_42773bd0…/sha256%3A83f70a…/index.html?replay=…`, `ready:true` |
| 7 | Certification declared static bundle | release run 32691323905 | **TRUE** | `Replay liveness: skipped (static replay bundle declared; …)` |
| 8 | Viewer executed & judged | featured match `cogmud.r3.e1` | **TRUE** | `loaded:true` @1833 ms; clocks `TURN 1/14` → `TURN 8/14` → `FINAL` |

**Verdict: 8/8 TRUE. No not-done items.**

Retry budget used: **checks 3, 4 and 5, one retry each**, all by the documented "different round"
approach — first run against round 3 at 05:26–05:27Z (checks 3 and 4 TRUE, check 5 FALSE), re-run
against round 4 at 05:47–05:48Z (all three TRUE). The round-3 evidence is preserved verbatim in the
Appendix. Checks 1, 2, 6, 7 and 8 passed first time and were not retried.

**Two findings carried forward for the coordinator (neither blocks any check):**

1. **Reply-parser strictness (Appendix A.3).** Round 3's episode logged
   `cogmud llm: seat 1 attempt 0 failed: input(6, 1) Error: EOF expected` twice and then
   `falling back to scripted decision`. Nim `parseJson`'s "EOF expected" means **trailing content
   after a complete JSON object** — the model emitted the required object and kept writing. It cost
   1 of 28 champion decisions in that episode and 0 in rounds 2 and 4, so it is intermittent, not
   systemic. Suggested fix (not applied — the verifier does not edit code): parse the first
   balanced JSON object in the reply, or strip a ```json fence, instead of requiring EOF after the
   object.
2. **Viewer name-swap inconsistency (check 8).** The clock renders a filler seat as `BASELINE (2)`
   while the endcard, scorebug and beat buttons render seats 2–5 by their aliases — so the endcard
   crowns `Ratchet WALKED OUT RICHEST` directly beneath a clock reading `BASELINE (2)`: the same
   seat under two names in one frame. Cosmetic; a phase-30 item-14 style legibility item.

**Independent corroboration the coworld is live:** between rounds 3 and 4, two outside players
entered the league unprompted — `relh` (`co-gas-cogmud-commission-runner-relhalpha:v1`) and
`richard` (`co-gas-cogmud-commission-runner-richard:v1`) — and both played round 4 with 0/14
scripted acts (check 3, check 4).

---

## Appendix A — superseded round-3 evidence (kept for the record)

These are the **first** runs of checks 3, 4 and 5, executed 2026-08-24T05:26–05:27Z against
round 3 (`round_578a4ce5-cc9d-45a0-93b6-6b8cc8eef912`, `ereq_1151194f-c9ea-4ce8-8d76-51f6b3af1d7b`,
replay `632e1ef6-7d66-4ad6-afd7-67aa2e772f97.replay`), which was the latest completed round at that
moment. They are superseded above by the round-4 re-runs, and are reproduced **verbatim and
unedited** — A.1 and A.2 passed and remain valid history; A.3 is the one FALSE result of this
verification and its diagnosis is the LEARNINGS material.

Round 3's replay is also the **featured match** that checks 6 and 8 exercised, so A.2's event
excerpts are the record the rendered frames in check 8 were reconciled against.

### A.1 — check 3 against round 3 (first run, 05:26:33Z; superseded) — was **TRUE**
...
### A.2 — check 4 against round 3's replay (first run, 05:26:45Z; superseded) — was **TRUE**
...
### A.3 — check 5 against round 3's log (first run, 05:26:58Z; superseded) — was **FALSE**

> **Superseded by the round-4 re-run in check 5 above, which is CLEAN.** Kept verbatim because the
> diagnosis below (Nim `parseJson` "EOF expected" = trailing content after a complete JSON object)
> is the actionable finding, and because the fleet cross-check documents that cogmud's fallback was
> *not* the platform-wide Bedrock symptom bullwhip and parley show.
