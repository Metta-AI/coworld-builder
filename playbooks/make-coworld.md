...
| Speech bubbles / captions render as slivers or read cut short, everything green | text drawn at a negative coordinate: the string was laid out relative to something else (a bubble growing upward from a cog at the top of the arena) with no room reserved for it. A canvas accepts the draw silently, so the load signal, the soak and the screenshot all pass. Reserve a **band in the layout** sized from the server's own cap on that string (`MaxSayLen`), measured in the font it is drawn in, and run `viewer_smoke.mjs --strict-text-bounds` in the wasm-viewer job — for a fixed arena `canvas_text.outside` must be 0 (cogchemists, 2026-08-24) |
