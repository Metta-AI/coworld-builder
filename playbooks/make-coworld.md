...
| phase-60 check 5: `cut off at max_tokens` with the cap already generous (≥900) | the model spends the whole budget on preamble before the JSON. Prefill the assistant turn with `{` (both Anthropic Messages and Bedrock invoke accept it), re-prefix the reply before parsing, and guard a provider that echoes the prefill; do not raise the cap — that trades max_tokens fallbacks for timeout fallbacks (cogame-procgen 0.1.2, 2026-08-28) |
| after a mid-run re-release, the next completed round still greps dirty | rounds scheduled before the new version went canonical run the old image (procgen rounds 7–9 mixed versions). Read each episode's `coworld_id` from `GET /episode-requests/<id>` and judge check 5 only on episodes from the new cow_id |
...
| Manifest build: `KeyError: 'image'` in the coworld CLI's `bundle.py` | the image belongs at `game.runnable.image`, never `game.image` — the CLI only reads it inside `runnable` (cogame-continuous-control 0.1.0, 2026-08-29) |
| Upload 400 `game runnable may not declare a cpu limit; only player pods honor one` | drop `game.runnable.resources.limits` entirely — resource limits are for `player[]` pods only (cogame-continuous-control 0.1.1, 2026-08-29) |
| phase 50 seeds the Nth league of one coworld and rounds run, but the coworld's spend cap is missed within the day | the budget is per **coworld**, not per league: `GET /coworlds/<name>/budget` before seeding, and set `round_interval_minutes` to whatever the sibling leagues use rather than the prompt's 15. Nine battlecode year leagues at ~0.112 USD/round already spent 17.59 of 20.00 USD/day; a tenth at 15-minute cadence would have added ~96 rounds/day (battlecode-2024, 2026-09-11) |
| phase 75 dispatch 1 fails `unplaced leagues (add them to CITIES in places.mjs): a, b, c` naming leagues that are not yours | `build.mjs` refuses while ANY live league is unplaced, so this is routine on a busy fleet, not a defect: place them too in `extra_cities` on dispatch 2 and name them in `log.md`. Budget one extra dispatch of the three for it (battlecode-2024 placed five, 2026-09-11) |
| an atlas dot lands inside another coworld's dashed territory ring | `tools/atlas_spot.py` avoids **cities** only, not `TERRITORIES`. Exclude `circ(cx,cy,r)` entries by `r + 6` (its arguments are overview units even though it emits image pixels — hence `raw: true`) and sample `path:` territories' cubic beziers into a polygon first. When your coworld already owns a territory with a lattice of sibling dots, skip the sweep and fill the lattice hole (battlecode-2024 at 202,270, 2026-09-11) |
| a sub-agent leg dies in-sandbox three times leaving nothing on disk, with no error to triage | pin **incremental writing** in every brief — "create the skeleton file as your first action, append each item as you trace it" — so a dead thread leaves a recoverable partial. If it still dies, say in the phase-90 ask that the same brief completes outside the sandbox when that is what the evidence shows; battlecode-2024's phase 30 was unblocked by an operator running the identical reviewer brief locally (2026-09-11) |
| A chrome/label fix (killfeed, endcard wording) that greps absent from the replay bytes in BOTH versions | beat labels are computed at **view** time by `broadcast.nim`'s `beatsFor*`, compiled into the wasm viewer bundle — no label string is ever written into a `.replay`. Prove such a fix from a rendered frame reconciled against the raw event, not a byte grep; it still needs a release (the bundle is keyed to `<cow_id>/<manifest_hash>`) and it applies retroactively to old replays (cogame-battlecode 0.8.2, 2026-09-09) |
| `git push` starts failing mid-session with `Password authentication is not supported` while `gh api` and `git fetch` still work | the github.com credential helper `!gh auth git-credential` base64-encodes the token inside a Basic header so egress cannot swap it. Push with `git -c credential.helper='' -c credential.helper=anthropic push`; where that binary is absent, land commits through the Git Data API (blob → tree → commit → ref). Never force-push either way (2026-09-09) |
...
| Push-triggered `upload-coworld` in ci.yml publishes an uncertified version that becomes canonical, racing `coworld-release.yml` (409 on the release's own upload) | once the registry has rows for the game, gate that job on the `UPLOAD_REQUIRED` repo variable — publishing is the release workflow's job (derks-gym 0.1.1, 2026-08-28) |
| Phase-60 check 8 returns loaded:true with clock/scorebug/scrub all null on a moba/emscripten-lineage viewer | the shell names its scrubber `#seek` and prefixes readouts (`#derk-clock`); fixed 2026-08-28 — `templates/tools/ci/viewer_smoke.mjs` probes both lineages (`#scrub|#seek`, `#clock|[id$="-clock"]`) |
| `trigger-round` 200 but no new round row, after a run of rounds that failed `only N/M planned slots produced scoring evidence` | the ladder tripped **its own breaker**: three consecutive failed rounds set `rounds_paused_at` on the league (read it from `GET /rounds/<id>` → `.division.league.rounds_paused_at`). Nothing runs until you `POST /leagues/$L/rounds-paused {"paused":false}` and trigger again. The slot failures themselves are usually platform-side — check the episode with `GET /rounds/<id>/episode-requests` then `GET /episode-requests/<ereq>`: `error_type: "crash"` with `"RemoteProtocolError (HTTP status unavailable)"` and `episode_id: null` is transport, not your game (battlecode bc19, 2026-09-11) |
| Is the platform having an incident, or is it my coworld? | measure it: page `GET /rounds?limit=200` through `next_cursor` and bucket every round by hour into completed vs failed-with-`scoring evidence`. Platform-wide baseline is 93-100 % completed; during the 2026-09-11 incident the same measure read 20 % for an hour and recovered to 91 % inside 40 minutes. Corroborate with an **unrelated** coworld family failing the identical single certification step (that day: `vanilla-wow`/`persistent-wow` 0.1.298 failed `smoke-episode` with `player websocket disconnected` while battlecode failed it with `ReadError`). Both readings are cheap and neither needs a dispatch |
| `trigger-round` 200 but `GET /rounds` stays `{"entries":[]}` forever — no round row, no error | league credit pool at 0 (`GET /leagues/$L/owner-status` → `credits.pool_credits`). Fund it: `POST /leagues/$L/reward-pool/grants` (100 credits, idempotency_key) + `PUT /leagues/$L/reward-pool/drip` `{"daily_drip_credits":100,"max_balance_credits":300}`, then trigger again (phase 50 step 7b; three runs Blocked five days on this, 2026-08-29→09-03) |
...
| Cert smoke-episode `did not answer a WebSocket Ping with Pong` on a fork whose episode is still running | the fork of coworld-ctf's `server.nim` lost the `Ping → Pong` branch in `websocketHandler` — second occurrence (lux-ai 0.1.0, snake-royale 0.1.0). Restore `socket.send(message.data, Pong)` on `Ping`, and guard NOTHING else: a `kind != TextMessage` guard drops the player's binary registration frames (ctf-lineage players register via `BinaryMessage`; docker-smoke reads "seat N never registered"). |
...
| Phase 40's three release dispatches all fail at `Upload the Coworld` (hosted smoke) with `certify.ok: true` | a degraded platform fleet, not your game — hosted smoke needs 100 % of its episodes on a cold-pulled image. Confirm with `GET /v2/rounds?limit=60` (unrelated coworlds failing `only <n>/<m> planned slots` in the same minutes). On a repo whose coworld version line is SHARED by several runs (cogame-battlecode's ten years), check whether the canonical row already carries your variant and policies and **adopt it** instead of spending a fourth dispatch — the artifact only has to certify the coworld the league actually runs (battlecode 0.11.2, 2026-09-11) |
