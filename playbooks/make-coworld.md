...
| docker-smoke green but a WARNING about falling back to `<SEATS>` | the cert fixture has no `num_agents`, so the smoke ran the wrong seat count. Green here is meaningless — fix the fixture. Grep the smoke log; never trust the job colour alone. |
